...
import { InMemoryQueue, QueueJob } from '../utils/inMemoryQueue'
...
// Singleton in-memory queue. Routes (upload.ts) push into this; the processor
// below pulls from it sequentially.
export const uploadQueue = new InMemoryQueue<UploadJobData>(UPLOAD_QUEUE_NAME)

const processUploadJob = async (job: QueueJob<UploadJobData>): Promise<void> => {
  ...
  try {
    ...
    throw error // Rethrow so the queue logs the failure
  }
}

export const startUploadWorker = (): void => {
  uploadQueue.setProcessor(processUploadJob)
  logger.info(`👷 [Worker] Listening to in-memory queue: ${UPLOAD_QUEUE_NAME}`)
}
